 ...
 * Send a SNMP trap (SNMPTrapAction)
 ...
 * Send a SNMP trap (SNMPTrapResponseAction)
...
`snmpd` can be instructed to send `SNMPv2` traps to a remote `SNMP` trap received by adding the
 ...
 * Built-in Actions for Rules:
    ...
    * `SNMPTrapAction([reason])`: send a SNMP trap, adding the optional `reason` string as the query description
    * `SNMPTrapResponseAction([reason])`: send a SNMP trap, adding the optional `reason` string as the response description
