...
While `protobufServer()` only exports the queries and responses made to the recursor from the clients, `outgoingProtobufServer()` can be used to
export outgoing queries made by the recursor to authoritative servers, along with the corresponding responses. 
